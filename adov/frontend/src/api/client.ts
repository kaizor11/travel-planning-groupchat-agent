...
/**
 * Fetch wrapper that retries once with a force-refreshed Firebase ID token on 401.
 * Handles the edge case where a token expires between scheduled refreshes (clock skew,
 * tab sleeping) so the user doesn't end up with a broken UI.
 */
...
export async function sendMessage(
  ...
  const res = await fetchWithAuth(
    `${API_BASE}/api/trips/${tripId}/messages`,
    {
      method: 'POST',
...
export async function confirmWishpool(
  ...
  const res = await fetchWithAuth(
    `${API_BASE}/api/trips/${tripId}/wishpool`,
    {
      method: 'POST',
      headers: authHeaders(idToken),
      body: JSON.stringify({
...
export async function joinTrip(tripId: string, idToken: string): Promise<void> {
  const res = await fetchWithAuth(
    `${API_BASE}/api/trips/${tripId}/join`,
...
export async function updateCalendarToken(accessToken: string, idToken: string): Promise<void> {
  const res = await fetchWithAuth(
    `${API_BASE}/api/users/me/calendar-token`,
    {
      method: 'PUT',
      headers: authHeaders(idToken),
...
export async function generateProposals(tripId: string, idToken: string): Promise<ProposalData[]> {
  const res = await fetchWithAuth(
    `${API_BASE}/api/trips/${tripId}/proposals/generate`,
    { method: 'POST', headers: authHeaders(idToken) },
    idToken,
  )
...
export async function getProposals(tripId: string, idToken: string): Promise<ProposalData[]> {
  const res = await fetchWithAuth(
    `${API_BASE}/api/trips/${tripId}/proposals`,
    { headers: { Authorization: `Bearer ${idToken}` } },
    idToken,
  )
...
export async function castVote(
  ...
  const res = await fetchWithAuth(
    `${API_BASE}/api/trips/${tripId}/proposals/${proposalId}/vote`,
    {
      method: 'POST',
      headers: authHeaders(idToken),
...
export async function getFreeBusy(
  ...
  const res = await fetchWithAuth(
    `${API_BASE}/api/calendar/freebusy`,
    {
      method: 'POST',
      headers: authHeaders(idToken),
